...
t.test('ignore missing optional deps', async t => {
  ...
  t.test('--json', t => {
    ...
    ls.exec([], (err) => {
      ...
      const problems = result.problems.map(p => p.split(npm.prefix).join('{project}'))
  ...
  t.test('--parseable', t => {
    ...
    ls.exec([], (err) => {
      t.match(err, { code: 'ELSPROBLEMS' })
      t.matchSnapshot(result.split(npm.prefix).join('{project}'), 'ls --parseable result')
      t.end()
    })
  })
  t.test('human output', t => {
    ...
    ls.exec([], (err) => {
      t.match(err, { code: 'ELSPROBLEMS' })
      t.matchSnapshot(result.split(npm.prefix).join('{project}'), 'ls result')
      t.end()
    })
  })
})
